...
- ⏱️ **阅读时间**：20-30分钟。
- 📚 **知识要点**：
    - SFT 的核心概念与损失函数（Cross-Entropy、Z-loss）及其适用场景。
    - 全参数微调（FFT）常见参数的作用与调参逻辑（模型、方法、数据、训练与分布式设置）。
    - vLLM 离线推理脚本 `vllm_infer.py` 的关键参数含义（采样、并行、多模态、输出评测）。
    - 训练与推理阶段的实战注意事项：显存-吞吐权衡、可复现性、评估指标解读。
