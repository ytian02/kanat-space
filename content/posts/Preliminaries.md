...
定义为从当前时刻的状态开始到终止状态结束时所有奖励的之和，考虑到未来奖励的不确定性，引入了折扣因子$\gamma$$$U_t=R_t+\gamma R_{t+1}+\gamma^2 R_{t+2}+\cdots=\sum\limits_{k=0}\gamma^kR_{t+k},$$注意$U_t$是一个随机变量，依赖于未来的状态和动作。
